function [her] = f_her2_weight_thres_NOnugget(x_cal, y_cal, z_cal, her)
%% function to optimize the weights of the classes
% optimizing AND/OR with weights not continuous,  optimizing the CDF based on a threshold
...
% - 2020/05/29 Stephanie Thiesen: no nugget version (changed slope calculation)
...
fobj = @(w_OR) f_DKL_w_OR_thres([1; w_OR(:)], z_cal', classes_obs_target_opt, pmf_z_target_given_neigh_opt, her.edges_z, her.z_thresh); %to avoid w multiples of 10, set the 1st class weight to 1 
...
[her.DKL_w_OR, ~, ~] = f_DKL_w_OR_thres(her.best_w_OR, z_cal', classes_obs_target_opt, pmf_z_target_given_neigh_opt, her.edges_z, her.z_thresh); % step 5: Normalized weights & step 7: OR PMF - Cross-validation
...
her.w_OR_slope = NaN(1,length(her.best_w_OR));
for i = 2:length(her.best_w_OR)
    her.w_OR_slope(i) = (her.best_w_OR(i) - her.best_w_OR(i-1)) / (her.edges_distance_classes_range(i) - her.edges_distance_classes_range(i-1));    
end
...
fobj = @(w_AND) f_DKL_w_AND_thres(w_AND, z_cal', classes_obs_target_opt, pmf_z_target_given_neigh_opt, her.edges_z, her.z_thresh); %to avoid w multiples of 10, set the 1st class weight to 1 
...
[her.DKL_w_AND, ~, ~] = f_DKL_w_AND_thres(her.best_w_AND, z_cal', classes_obs_target_opt, pmf_z_target_given_neigh_opt, her.edges_z, her.z_thresh); % step 5: Normalized weights & step 7: OR PMF - Cross-validation
...
% slope of the last bin (full dataset) equals to the penultimate
her.w_AND_slope = NaN(1,length(her.best_w_AND));
for i = 2:length(her.best_w_AND)
    her.w_AND_slope(i) = (her.best_w_AND(i) - her.best_w_AND(i-1)) / (her.edges_distance_classes_range(i) - her.edges_distance_classes_range(i-1));    
end
...
%         weights_cont_and_obs_nn(obs,target) =  her.best_w_AND(classes_obs_nn(obs,target)) + her.w_AND_slope(classes_obs_nn(obs,target)) * ( mat_euc_distance_xy_nn(obs,target) - her.edges_distance_classes_range(classes_obs_nn(obs,target)) ); %continuous OR weights
        weights_cont_or_obs_nn(obs,target) =  her.best_w_OR(classes_obs_nn(obs,target));
        weights_cont_and_obs_nn(obs,target) =  her.best_w_AND(classes_obs_nn(obs,target));
...
for w = 1 : size(w_alpha_beta,1) %for each set of AND_OR weights
    prob_ = cell(1,size(pmf_alpha_beta_nn,2));
    for i = 1:size(pmf_alpha_beta_nn,2)
        pmf_ = pmf_alpha_beta_nn{w,i};
        cmf_ = [0 cumsum(pmf_)];
        p_ = interp1(her.edges_z, cmf_, her.z_thresh);
        prob_{1,i} = [ p_, 1-p_ ];
    end
    edges_thres = [ her.edges_z(1) her.z_thresh her.edges_z(end) ];
    PMF_true = ones(1,size(z_cal,1));
    DKL_w_alpha_beta_(w) = f_performance_prob(z_cal', prob_, PMF_true, edges_thres);
end
