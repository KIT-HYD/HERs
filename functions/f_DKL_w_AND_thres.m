function [DKL_w_AND, varargout] = f_DKL_w_AND_thres(weights_and, z_target_opt, classes_obs, pmf_diff_z_plus_z, edges_z, z_thresh)
...
% Step 2: AND PMF - Cross-validation
    ...
    for target = 1 : size(classes_obs,2) %for each target
        ...
        [ pmf_ ] = f_loglinear_aggregation(pmfs_, weights_and_); %save the predicted z PMF of the target
        cmf_ = [0 cumsum(pmf_)];
        p_ = interp1(edges_z, cmf_, z_thresh);
        pmf_AND{1,target} = [ p_, 1-p_ ];
    end
    edges_thres = [ edges_z(1) z_thresh edges_z(end) ];
    PMF_true = ones(1,size(classes_obs,1));
    [DKL_w_AND] = f_performance_prob(z_target_opt, pmf_AND, PMF_true, edges_thres);
