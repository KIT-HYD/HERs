function [pmf_pred_nn, target_idx_zero_neigh_pred] = f_her3_predict_NOnugget(x_cal, y_cal,z_cal, x_target, y_target, her)
%% function to predict z PMFs using HER
% 2: version with steep slope for the 1st class (for solving undesired
% uncertainty when predicting a value at the location of the observation)
...
% -------------- Version --------------
% - 2020/05/20 Stephanie Thiesen: intial version
% - 2020/06/16 Diego Vieira: truncate target pmf when outside range of edges_z
...
%     weights_and_cont_pred_nn = zeros(her.n_neighbor,length(x_target));
    for target = 1:length(x_target)
        for obs = 1:her.n_neighbor
            if mat_euc_distance_obs_target_pred_nn(obs,target) == 0
                weights_or_cont_pred_nn(obs,target) = 1e16;
                weights_and_cont_pred_nn(obs,target) = 1e16;
            elseif mat_euc_distance_obs_target_pred_nn(obs,target) <= her.edges_distance_classes_range(2)
                weights_or_cont_pred_nn(obs,target) = her.best_w_OR(classes_obs_target_pred_nn(obs,target)) * her.edges_distance_classes_range(2) / mat_euc_distance_obs_target_pred_nn(obs,target);
                weights_and_cont_pred_nn(obs,target) = her.best_w_AND(classes_obs_target_pred_nn(obs,target)) * her.edges_distance_classes_range(2) / mat_euc_distance_obs_target_pred_nn(obs,target);
            else
%             weights_or_pred_nn(obs,target) = her.best_w_OR(classes_obs_target_pred_nn(obs,target)); %discrete OR weights
                weights_or_cont_pred_nn(obs,target) =  her.best_w_OR(classes_obs_target_pred_nn(obs,target)) + her.w_OR_slope(classes_obs_target_pred_nn(obs,target)) * ( mat_euc_distance_obs_target_pred_nn(obs,target) - her.edges_distance_classes_range(classes_obs_target_pred_nn(obs,target)+1)); %continuous OR weights
%             weights_and_pred_nn(obs,target) = her.best_w_AND(classes_obs_target_pred_nn(obs,target)); %discrete OR weights
                weights_and_cont_pred_nn(obs,target) =  her.best_w_AND(classes_obs_target_pred_nn(obs,target)) + her.w_AND_slope(classes_obs_target_pred_nn(obs,target)) * ( mat_euc_distance_obs_target_pred_nn(obs,target) - her.edges_distance_classes_range(classes_obs_target_pred_nn(obs,target)+1) ); %continuous OR weights
            end
        end
    end
...
% aggregation
    ...
    diff_n_bins_shift = int32(her.n_bins_z * her.n_bins_z_per_bin_shift - her.n_bins_shift); %how many small bins we will need to add (before and/or after) to have "numbins_z_per_bin_shift" bins for each bin in the final z PMF
    for target = 1:numel(x_target)
        % z PMF of the known observations(obs_diff_z + z)
        for obs = 1:her.n_neighbor
            class_ = classes_obs_target_pred_nn(obs,target); 
            bins_shift = int32(round((her.edges_diff_z_shift(1) + z_cal_nn(obs,target) - her.edges_z(1)) / her.binwidth_shift));
%             idx_bins_shift = reshape( ...           % index matrix to find which bins from the shift PMF go into each bin of the z PMF
%                   [ones(1,bins_shift) 2:her.n_bins_shift+1 ones(1,diff_n_bins_shift - bins_shift)], ... %fill edges with 1 (zero bin)
%                   her.n_bins_z_per_bin_shift, her.n_bins_z); % matrix with one column per z PMF bin and "numbins_z_per_bin_shift" rows
            bins_trunc_left = max(0,-bins_shift);                   % discard bins outside to the left of edges_z
            bins_trunc_right = max(0,bins_shift-diff_n_bins_shift); % discard bins outside to the right of edges_z
            idx_bins_shift = reshape( ...           % index matrix to find which bins from the shift PMF go into each bin of the z PMF
                  [ones(1,bins_shift) bins_trunc_left+2:her.n_bins_shift+1-bins_trunc_right ones(1,diff_n_bins_shift - bins_shift)], ... %fill edges with 1 (zero bin)
                  her.n_bins_z_per_bin_shift, her.n_bins_z); % matrix with one column per z PMF bin and "numbins_z_per_bin_shift" rows
            ...
            if (bins_trunc_left > 0)
                warning('Discarding %.2f%% of pmf outside to the left of the range of edges_z (target x: %.2f, y: %.2f, discarted from z_obs(%d): %.2f)', 100*sum(pmf_(2:1+bins_trunc_left)), x_target, y_target, obs, z_cal_nn(obs,target));
            elseif (bins_trunc_right > 0)
                warning('Discarding %.2f%% of pmf outside to the right of the range of edges_z (target x: %.2f, y: %.2f, discarted from z_obs(%d): %.2f)', 100*sum(pmf_(end-bins_trunc_right:end)), x_target, y_target, obs, z_cal_nn(obs,target));
            end
        end
