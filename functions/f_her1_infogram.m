function [her] = f_her1_infogram(x_cal, y_cal,z_cal, her)
...
her.n_pairs_by_bin_by_class = obs_diff_z_by_class;
for i = 1 : length(her.edges_distance_classes)-1 %for each class
    idx = her.mat_euc_distance_xy > her.edges_distance_classes(i) & her.mat_euc_distance_xy <= her.edges_distance_classes(i+1); %find the diff_z within the current lag class
    obs_diff_z_by_class{i} = her.mat_diff_z(idx); %save the diff_z 
    her.n_pairs_by_bin_by_class{i} = histcounts(obs_diff_z_by_class{i},her.edges_diff_z);
...
% compute bin edges
n_bins_left = -floor((3*min_diff_z + min_z)/ her.binwidth_z);
n_bins_right = floor((3*max_diff_z + max_z)/ her.binwidth_z + 1); %edge <= x < edge
